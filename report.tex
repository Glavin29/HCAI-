...
\title{FairHire AI: A Human-Centered Machine Learning Framework for Fair Resume Screening}
...
\begin{abstract}
Algorithmic hiring systems support recruitment by reducing manual screening workloads, but they risk reproducing social bias and concealing consequential decision logic. This paper presents FairHire AI, a human-centered resume screening framework utilizing RoBERTa semantic representations—a transformer-based pipeline that extracts dense contextual embeddings from resume texts, combined with a logistic regression classifier to predict candidate role consistency. Because the training data represents historical profiles rather than explicit hiring outcomes, the classifier's predicted probability is utilized as a profile-consistency ranking signal. The system audits the ranked shortlist across gender categories using selection rates, impact ratios, and score parity, and incorporates a qualitative human-centered AI (HCAI) audit of proxy-risk variables. The results demonstrate that semantic resume screening achieves a competitive fairness profile (0.94 disparate impact ratio and a 1.14\% selection gap) and that fair AI in recruitment requires a socio-technical approach combining representation-aware modeling, interface transparency, and human oversight.
\end{abstract}

\keywords{Algorithmic Fairness, Human-Centered AI, Resume Screening, Explainable Machine Learning, Algorithmic Auditing, RoBERTa, Transformer Embeddings, Socio-technical Systems}

\maketitle

\section{INTRODUCTION}
Hiring is a high-impact decision domain. Recruitment decisions affect employment access, income, professional mobility, and social inclusion. As organizations increasingly use algorithmic systems to screen resumes, fairness becomes a central concern. A model may appear objective because it processes structured data, but it can still reproduce historical inequalities, biased data collection practices, proxy discrimination, and unequal opportunity structures. Under regulations such as the European Union AI Act, automated hiring tools are classified as ``high-risk'' AI systems, requiring strict compliance regarding transparency, data governance, robustness, and human oversight.
...
FairHire AI was developed as a Human-Centered AI (HCAI) research framework. Rather than automating recruitment decisions, the primary objective is to build an interactive, transparency-first prototype that demonstrates how resume screening can be made auditable, explainable, and fairness-aware. The system avoids using candidate names as model inputs to prevent direct bias. It also excludes gender from prediction features, utilizing it only after scoring for fairness auditing and mitigation. By integrating these quantitative tools with a qualitative HCAI audit framework, the system aims to support recruiters in making more responsible, human-in-the-loop decisions.

\subsection*{Problem Statement}
\textit{Algorithmic hiring systems can efficiently parse candidate attributes, but they often present static, generic outputs and lack transparent fairness reporting. This limits their utility for stakeholders with diverse operational and ethical requirements. Without proper auditing, models reproduce systemic bias and provide no visibility into how specific ranking scores are computed, reducing effectiveness, trustworthiness, and ethical alignment in hiring decisions.}
...
\begin{itemize}
    \item \textbf{HR Recruiters and Hiring Managers} require actionable decision support. They need candidate scores accompanied by clear, local explanations of key profile signals (skills, experience, certifications) to verify suitability and prevent automated bias.
    \item \textbf{Candidates} require transparency and accountability. They need assurance that protected characteristics (such as gender and name) are excluded from scoring, and expect clear pathways to understand how they are represented.
    \item \textbf{Compliance Officers and Auditors} require systematic auditing tools. They rely on quantitative metrics (selection rates, impact ratios, score parity) and qualitative HCAI audit trails to ensure compliance with emerging standards like the EU AI Act and NIST SP 1270.
\end{itemize}
The goal of this research is to bridge this socio-technical divide and contribute to the development of human-centered AI (HCAI) in recruitment.

\subsection*{Objectives}
\begin{itemize}
    \item \textbf{Implement Candidate-Level Scoring}: Train a role-consistency classifier using pre-trained RoBERTa transformer representations to parse candidate profiles.
    \item \textbf{Enforce Demographic Feature Exclusion}: Exclude candidate names and gender from prediction features to prevent direct bias.
    \item \textbf{Generate Local Explanations}: Provide human-readable summaries of candidate profile signals and classification outputs for oversight.
    \item \textbf{Audit Statistical Parity}: Track selection rates and score distributions across gender categories to monitor adverse impact.
    \item \textbf{Formulate a Qualitative Audit}: Assess potential proxy risks associated with non-protected attributes (e.g., graduation year, institute name).
    \item \textbf{Deploy Post-Processing Mitigation}: Integrate representation-aware rebalancing to examine the mathematical trade-offs between selection accuracy and group parity.
\end{itemize}
...
Barocas, Hardt, and Narayanan~\cite{barocas2023} argue that algorithmic fairness cannot be understood only as an optimization problem. Fairness criteria involve normative choices, and different definitions can conflict. Their work also emphasizes that discrimination may arise from data, measurement, labels, and institutional context. For instance, attempting to satisfy equal selection rates (demographic parity) may conflict with predictive parity if underlying group distributions differ in the training set. Raghavan et al.~\cite{raghavan2020} evaluate industry practices in algorithmic hiring and discuss how bias auditing is marketed vs. implemented in commercial systems.

\subsection{Machine Learning-Based Screening Applications}
Automated resume screening typically relies on extracting structured text features and mapping them to professional targets. However, historical training data often reflects pre-existing human biases. For instance, in 2018, Amazon scrapped an AI recruiting tool because it had learned to penalize resumes containing the word ``women's''~\cite{dastin2018}. Consequently, the model acts as an ``equity-washing'' mechanism, reinforcing systemic bias under the guise of algorithmic objectivity.
...
To support accountability, systems must provide structured, standard documentation of a model's performance, training data, and ethical considerations. In interactive settings, user interface design must present these statistics dynamically, allowing recruiters to inspect demographic impact in real time.
...
We employ a pre-trained RoBERTa transformer model (\texttt{roberta-base}) to extract semantic representations from the candidate resume text. The textual fields (such as skills, certifications, and target job description) are unified into a single sequence $S$ and tokenized. Let $T(S)$ be the sequence of tokens, and let $\text{RoBERTa}(T(S))$ represent the output hidden states of the transformer encoder. The dense representation vector $\vec{v}(r) \in \mathbb{R}^{768}$ is extracted from the hidden state corresponding to the first pooling token ($[\text{CLS}]$):
\begin{equation}
